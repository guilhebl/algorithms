...
public class ChessMetric {
	...
	public static long howMany2(int size, int[] start, int[] end, int numMoves) {	
		int n = size;
		int m = numMoves;
		long dp[][][] = new long [n][n][m + 1];
		dp[start[0]][start[1]][0] = 1;
		...
		for (m = 1; m <= numMoves; m++) {
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < n; j++) {
					...
					// clockwise rotation starting from UP
					if (i > 0) {
						max = Math.max(max, dp[i-1][j][m-1]);
					}
					if (i > 0 && j < dp[i].length-1) {
						max = Math.max(max, dp[i-1][j+1][m-1]);	
					}						
					if (j < dp[i].length-1) {
						max = Math.max(max, dp[i][j+1][m-1]);	
					}						
					if (i < dp.length-1 && j < dp[i].length-1) {
						max = Math.max(max, dp[i + 1][j+1][m-1]);	
					}						
					...
					// L positions
					if (i > 1 && j > 0) {
						max = Math.max(max, dp[i-2][j-1][m-1]);	
					}						
					if (i > 0 && j > 1) {
						max = Math.max(max, dp[i-1][j-2][m-1]);	
					}						
					if (i > 1 && j < dp[i].length-1) {
						max = Math.max(max, dp[i-2][j+1][m-1]);	
					}						
